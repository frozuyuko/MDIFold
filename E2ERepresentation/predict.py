...
def pipeline(fastafile,ssfile,savefile_):
    ...
    for amodel in model_list:
        ...
        with torch.no_grad():
            N_cycle = 4
            m1_pre,z_pre = model.pred(msa,ss,base_x,N_cycle) 
            #predxs = model.pred(msa,ss,base_x,N_cycle)
            #coor = predxs[N_cycle-1].numpy()
            #np.save(savefile_+'_'+str(count),coor)
            count+=1
            np.save(savefile_+'_m1_pre_'+str(count),m1_pre.cpu().data.numpy()) 
            np.save(savefile_+'_z_pre_'+str(count),z_pre.cpu().data.numpy())
            # np.save(savefile_+'_z_pre_'+str(count),z_pre) 
            #return m1_pre, z_pre #updated for the modification (returning m atrix and sum of vector)
